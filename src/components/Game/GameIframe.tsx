...
export const GameIframe = ({ game }: GameIframeProps) => {
  ...
  const tips = [
    ...
    { text: 'Log in to Bit Legends to save favorites and track playtime!' },
      ...
      >
        {!introDismissed ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center bg-gradient-to-b from-black via-neutral-900 to-black p-6">
            ...
            <p className="italic text-gray-300 mb-5 max-w-md">
              {tips[tipIndex].text}
            </p>
            <button
          ...
          <>
            ...
            {isMobile && (
              <button
                ...
                className="absolute bottom-1 right-1 z-10 bg-black/60 text-white rounded px-4 py-1 text-xs sm:text-sm border border-white/20 hover:bg-black/10 transition-all"
                title="Fullscreen"
              >
                ⛶
              </button>
