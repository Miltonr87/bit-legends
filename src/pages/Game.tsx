...
import { ArrowLeft, Calendar, Users, Gamepad2, Info, Star, Share2, Instagram, MessageCircle } from "lucide-react";
...
const Game = () => {
  ...
  // Load star rating from localStorage
  useEffect(() => {
    if (game) {
      const savedRating = localStorage.getItem(`game_rating_${game.id}`);
      if (savedRating) {
        setStarRating(parseInt(savedRating));
      }
    }
  }, [game]);
  ...
  const handleStarClick = (rating: number) => {
    if (game) {
      ...
      toast({
        title: "Rating saved!",
        description: `You rated ${game.title} ${rating} star${rating !== 1 ? 's' : ''}`,
      });
    }
  };
  ...
  const iframeUrl = game.embedUrl || `https://www.retrogames.cc/embed/${game.embedId}-${game.slug}.html`;
    ...
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8">
        <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <Link to="/">
            <Button variant="outline" className="border-accent/50 hover:bg-accent/10 w-full sm:w-auto">
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back to Games
            </Button>
          </Link>
        ...
        <div className="grid lg:grid-cols-3 gap-4 sm:gap-8">
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6">
            <Card className="overflow-hidden border-2 border-accent/30 bg-card">
              <div className="bg-gradient-to-r from-primary/20 to-accent/20 p-3 sm:p-4 border-b border-accent/30">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    <h1 className="text-xl sm:text-3xl font-bold glow-text truncate">{game.title}</h1>
                    <p className="text-xs sm:text-base text-muted-foreground mt-1">{game.developer} • {game.year}</p>
                  </div>
                  <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="outline" size="icon" className="border-accent/50 h-8 w-8 sm:h-10 sm:w-10">
              ...
              <div ref={gameIframeRef} className="relative bg-black overflow-hidden" style={{ aspectRatio: '4/3' }}>
                <iframe
                  src={iframeUrl}
                  width="100%"
                  height="100%"
              ...
              <div className="p-3 sm:p-4 bg-muted/30 border-t border-accent/20">
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-foreground/80">Use arrow keys and Z, X, C keys</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Info className="h-4 w-4 text-accent flex-shrink-0" />
                    <span className="text-foreground/80">Press F for fullscreen</span>
                  </div>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-4 sm:p-6 border-2 border-border bg-card">
              <h2 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-accent">About This Game</h2>
